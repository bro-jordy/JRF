...
export async function POST(req: NextRequest) {
  ...
  const [{ data: budgets }, { data: transactions }, { data: categories }, { data: members }] =
    await Promise.all([
      supabase.from("budgets").select("category_id, amount").eq("household_id", household_id).eq("month", month),
      supabase
        .from("transactions")
      ...
      supabase.from("household_members").select("user_id").eq("household_id", household_id),
    ]);

  if (!budgets?.length || !members?.length) return NextResponse.json({ ok: true, sent: 0 });
  ...
  // Get push subscriptions for all household members
